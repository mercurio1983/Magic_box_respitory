...
run_exp::run_exp(const wxString& title) : wxFrame(NULL, wxID_ANY, title, wxPoint(600,350), wxSize(220,300))
{
	...
	wxButton *induction = new wxButton(this,ID_INDUCTION,wxT("&Induction"));
	...
	*/
	...
	grid_size = new wxGridSizer(6,2,2,2);
	...
	grid_size->Add(induction,0,wxEXPAND);
...
void run_exp::OnRun(wxCommandEvent& WXUNUSED(event)){
	...
	std::string str_1 = std::string(get_text_value(INDUCTION).mb_str());
	std::string str_2 = std::string(get_text_value(STOP_SIGNAL).mb_str());
	std::string str_3 = std::string(get_text_value(TEST_MOVEMENT).mb_str());
	std::string str_4 = std::string(get_text_value(BLACK_SCREEN).mb_str());
	std::string str_total = "./stereoviewGL_delay3D " + str_1 + " " + str_2 + " " + str_3 + " " + str_4;
...
void run_exp::SetInduction(wxCommandEvent& WXUNUSED(event)){
	...
	long max_value = wxAtoi(get_max_value(INDUCTION));
	wxNumberEntryDialog *dialog = new wxNumberEntryDialog(this,wxT("Write the induction time in s"),wxT("Induction time"),wxT("Induction time"), 10, 0, max_value);
...
void run_exp::SetStop_signal(wxCommandEvent& WXUNUSED(event)){
	...
	long max_value = wxAtoi(get_max_value(STOP_SIGNAL));
	wxNumberEntryDialog *dialog = new wxNumberEntryDialog(this,wxT("Write the stop signal time in s"),wxT("stop signal time"),wxT("stop signal time"), 2, 0, max_value);
	if(dialog->ShowModal() == wxID_OK){
...
void run_exp::SetTest_movement(wxCommandEvent& WXUNUSED(event)){
	...
	long max_value = wxAtoi(get_max_value(TEST_MOVEMENT));
	wxNumberEntryDialog *dialog = new wxNumberEntryDialog(this,wxT("Write the test movement time in s"),wxT("test movement time"),wxT("test movement time"), 3, 0, max_value);
	if(dialog->ShowModal() == wxID_OK){
...
void run_exp::SetBlack_screen(wxCommandEvent& WXUNUSED(event)){
	...
	long max_value = wxAtoi(get_max_value(BLACK_SCREEN));
	wxNumberEntryDialog *dialog = new wxNumberEntryDialog(this,wxT("Write the blackscreen time in s"),wxT("blackscreen time"),wxT("blackscreen time"), 5, 0, max_value);
	if(dialog->ShowModal() == wxID_OK){
...
void run_exp::print_value(wxCommandEvent& WXUNUSED(event)){
	...
	wxString str_1 = get_text_value(INDUCTION);
	wxString str_2 = get_text_value(STOP_SIGNAL);
	//long value_sync = wxAtoi(str_2);
	//wxString str_indicate;
	//if(!value_sync){ str_indicate = wxT("sync");}
	//else{str_indicate = wxT("async"); }
	wxString str_3 = get_text_value(TEST_MOVEMENT);
	wxString str_4 = get_text_value(BLACK_SCREEN);
